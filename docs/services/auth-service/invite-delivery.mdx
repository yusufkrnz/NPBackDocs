...
sequenceDiagram
    ...
    Auth->>DB: tenant_outbox_events (event_type=send_invite_email, status=pending) — TEK transaction'da
    Auth-->>SA: 201 (panelde SADECE "e-posta gönderildi" mesajı — link YOK)
    Note over Auth,SMTP: Poller worker ayrı çalışır — pending event'i backoff ile retry eder
    Auth-)SMTP: poller -> send_invite_email(event)
...
## E-posta gönderimi — SMTP, `.env`'den, outbox ile retry'li

:::info Karar değişikliği
Bu bölüm daha önce `BackgroundTasks.add_task` ile fire-and-forget gönderim yapıyordu (istek bloklanmaz ama başarısızlıkta otomatik retry de yoktu, sadece manuel "yeniden gönder" butonu vardı). Artık gönderim bir **outbox event**'i — poller worker backoff ile otomatik retry ediyor. Tam mekanizma: [Tenant Provisioning — Outbox Pattern](/guides/tenant-provisioning-outbox).
:::

`send_invite_email` fonksiyonunun kendisi (SMTP'ye ne gönderdiği) değişmedi — sadece **kim, ne zaman çağırdığı** değişti: artık router değil, outbox poller'ın handler'ı çağırıyor.

:::note Implementasyon durumu
`services/email_service.py` (`send_invite_email` + `send_invite_email_handler`) **kodlandı**, `core/outbox` motoruna kayıtlı ve smoke test'ten geçti — henüz canlı SMTP ile kasıtlı tetiklenmedi. `routers/admin.py` (aşağıdaki `POST /admin/tenants` örneği) henüz yazılmadı, `models/user.py`/`models/invitation.py`'ı bekliyor.
:::
...
async def create_tenant(payload: CreateTenantRequest, db: AsyncSession = Depends(get_tenant_session)):
    tenant, admin_user = await create_tenant_and_admin(db, payload)   # kritik, atomik (Postgres)
    raw_token = create_invitation(db, admin_user.id)

    # E-posta artık outbox event'i — AYNI transaction'da, atomik şekilde kaydedilir
    db.add(TenantOutboxEvent(
        tenant_id=tenant.id,
        event_type="send_invite_email",
        status="pending",
        payload={"email": admin_user.email, "raw_token": raw_token},
    ))
    await db.commit()
...
<CodeRail>
  <CodeEditor
    filename="core/outbox.py"
    code={`async def send_invite_email_handler(event: TenantOutboxEvent) -> None:
    await send_invite_email(
        to_email=event.payload["email"],
        raw_token=event.payload["raw_token"],
    )
    # başarısız olursa exception fırlar, poller attempt_count/backoff'u kendisi yönetir
    # (bkz. Tenant Provisioning — Outbox Pattern → Retry / backoff / dead-letter)`}
  />
</CodeRail>
...
- [Tenant Provisioning — Outbox Pattern](/guides/tenant-provisioning-outbox) — `send_invite_email` event'inin poller/retry/backoff mekanizması
