---
title: Ekip Daveti
sidebar_position: 5
description: Tenant admin'inin ekip arkadaşını davet etmesi ve davetin kabul edilmesi.
wide_toc: true
---

import ApiExample from '@site/src/components/ApiExample';
import CodeRail from '@site/src/components/CodeRail';

<CodeRail>
  <ApiExample
    method="POST"
    path="/team/invitations"
    requestHeaders={{ Authorization: 'Bearer <admin_access_token>' }}
    request={{ email: 'mehmet@akguntic.com' }}
    response={{ id: 'inv_7a21c9', message_code: 'tenant.invitation.sent' }}
  />
</CodeRail>

# Ekip Daveti

[Tenant Oluşturma](/use-cases/user-registration) sadece **ilk** kullanıcıyı (tenant admin'i), süper admin tarafından oluşturur. Sonraki her üye bu davet akışıyla eklenir — `POST /auth/accept-invite`, ilk admin'in de kullandığı **aynı** endpoint; açık bir "register" ile rastgele bir tenant'a katılmak mümkün değildir. Davet e-postasının token üretiminden SMTP gönderimine kadar tam mekanizması için bkz. [Davet Linki Teslimi](/services/auth-service/invite-delivery).

## Akış

```mermaid
sequenceDiagram
    participant Admin as Admin (Client)
    participant Tenant as Tenant Service
    participant DB
    participant Mail as E-posta servisi
    participant Invitee as Davet edilen (Client)

    Admin->>Tenant: POST /team/invitations {email}
    Tenant->>Tenant: current_user.role == admin mi?
    alt admin değil
        Tenant-->>Admin: 403 AuthorizationError (code: tenant.invite_requires_admin)
    else zaten üye
        Tenant-->>Admin: 409 ConflictError (code: tenant.email_already_member)
    else geçerli
        Tenant->>DB: Invitations kaydı oluştur (tenant_id, email, token_hash, TTL 7 gün)
        Tenant->>Mail: davet linki gönder
        Tenant-->>Admin: 201 {id, message_code}
    end

    Invitee->>Tenant: POST /auth/accept-invite {token, password, full_name}
    Tenant->>DB: token hash'ini doğrula (süre + is_used)
    Tenant->>DB: Users (tenant_id=davetteki tenant, role=member, is_active=true) oluştur
    Tenant->>DB: Invitations.is_used = true
    Tenant-->>Invitee: 201 {id, tenant_id}
```

## Neden davet kabulünde ayrıca e-posta doğrulama yok

[Kayıt olma](/use-cases/user-registration)'da hesap `is_active=false` ile başlar çünkü e-posta sahipliği doğrulanmamıştır. Davet akışında bu adım gereksiz — davet zaten o e-posta adresine gönderildi, linke tıklayıp token'ı sunabilmek e-postanın sahipliğini kanıtlar. Bu yüzden davetle katılan kullanıcı doğrudan `is_active=true` olarak oluşturulur.

## Hata / edge-case senaryoları

| Senaryo | Sonuç |
|---|---|
| Davet gönderen admin değil | `403 AuthorizationError`, `code: tenant.invite_requires_admin` |
| Davet edilen e-posta zaten bu tenant'ta üye | `409 ConflictError`, `code: tenant.email_already_member` |
| Davet edilen e-posta **başka bir tenant'ta** zaten kayıtlı | `409 ConflictError`, `code: auth.email_already_registered` — e-posta genel olarak tekildir (bkz. [Kayıt olma](/use-cases/user-registration)) |
| Davet token'ı süresi dolmuş (7 günden eski) | `401 AuthenticationError`, `code: tenant.invitation_expired` — admin yeniden davet göndermeli |
| Davet token'ı zaten kullanılmış | `401 AuthenticationError`, `code: tenant.invitation_already_used` |

## Sırada ne var

Davetle katılan kullanıcı doğrudan [Login ve token oluşturma](/use-cases/login-and-token-issuance) akışıyla giriş yapabilir — ayrıca bir aktivasyon adımı yoktur.
